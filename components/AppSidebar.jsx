...
import { FileExplorer } from "./custom/FileExplorer";
...
const AppSidebar = ({ children }) => {
  ...
  return (
    <>
      ...
      <div
        className={`h-full bg-slate-100 z-50 hidden md:block border-r shadow-sm transition-all duration-300 ${
          sideBar ? "w-[250px]" : "w-0"
        }`}
      >
        <div
      ...
      <div
        className={`fixed  inset-y-0 left-0 z-50 md:${isWorkspaceRoute ? "block" : "hidden"} bg-slate-100 shadow-lg transition-transform duration-300 w-[250px] ${
      ...
      <div
        className={`fixed inset-y-0 left-0 z-[100] md:hidden bg-slate-100 shadow-lg transition-transform duration-300 w-[250px] ${
